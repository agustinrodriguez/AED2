...
{
    int j = 0;
    LinkLinkIt::itPuntLinks it = sistema->linksOrdenadosPorAccesos(c);
    while(j < i)
    {
        ++j;
        it.Avanzar();
...
{
    int j = 0;
    LinkLinkIt::itPuntLinks it = LinkLinkIt::itPuntLinks(sistema->linksOrdenadosPorAccesos(c));
    while(j < i)
    {
        ++j;
        it.Avanzar();
    }
    return it.SiguienteCantAccesos();
}
