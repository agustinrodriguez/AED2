...
\subsubsection{\Large Operaciones B\'asicas}
	\textbf{categorias} (\textbf{in} s: lli) $\longrightarrow$ res: acat\\\\
	Pre $\equiv$ {true}\\
...
$\hspace*{10mm}$\textit{familia}:diccTrie(\textit{padre}:string,tupla(\textit{abuelo}:string,\textit{hijos}:conj(string),\textit{id}:nat,\textit{altura}:nat)),\\)\\
	...
	\subsubsection{\Large Invariante de Representaci\'{o}n}
	  ...
	  \begin{enumerate}
		...
		\item Para todo '\textbf{padre}' que exista en '\textbf{familia}' debera ser o raiz o pertenecer a algun conjunto de hijos de alguna clave '\textbf{padre}'
		...
		\item No hay '\textit{id}' repetidos en el '\textit{familia} y son todos consecutivos.
...
\end{flushright}
	...
	\subsubsection{Algoritmos}
		...
		\textbf{ICATEGORIAS} (\textbf{in} ac: acat) $\longrightarrow$ res: conj(categoria)\\
		res $\leftarrow$ claves(ac.familia)   // O(ALGO) 
		
		\textbf{IRAIZ} (\textbf{in} ac: acat) $\longrightarrow$ res: categoria\\
		res $\leftarrow$ ac.raiz // O(1)
		
		\textbf{IPADRE} (\textbf{in} ac: acat, \textbf{in} h: categoria) $\longrightarrow$ res: categoria\\
		res $\leftarrow$ (obtener(h,ac.familia)).abuelo // O(ALGO)
		
		\textbf{IID} (\textbf{in} ac: acat, \textbf{in} c: categoria) $\longrightarrow$ res:nat\\
		res $\leftarrow$ (obtener(c,ac.familia)).id // O(ALGO)
		
		\textbf{INUEVO} (\textbf{in} c: categoria) $\longrightarrow$ res:acat\\\\
		
		\textbf{IAGREGAR} (\textbf{in/out} ac: acat,\textbf{in} c: categoria, \textbf{in} h: categoria )\\\\
		
		\textbf{IALTURA} (\textbf{in} ac: acat) $\longrightarrow$ res:nat\\
		res $\leftarrow$ ac.alturaMax // O(ALGO)
		                                                                  
		\textbf{IESTA?} (\textbf{in} c: categoria,\textbf{in} ac: acat) $\longrightarrow$ res:bool\\\\
		res $\leftarrow$ def?(c,ac.familia) // O(ALGO)
		
		\textbf{IESSUBCATEGORIA} (\textbf{in} ac: acat, \textbf{in} c: categoria,\textbf{in} h: categoria) $\longrightarrow$ res:bool\\
		
		\textbf{IALTURACATEGORIA} (\textbf{in} ac: acat, \textbf{in} c: categoria) $\longrightarrow$ res:nat\\
		res $\leftarrow$ (obtener(c,ac.familia)).altura // O(ALGO)
		
		\textbf{IHIJOS} (\textbf{in} ac: acat, \textbf{in} c: categoria) $\longrightarrow$ res:conj(categoria)\\
