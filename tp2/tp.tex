...
ELSE *((obtener(l,s.accesosXLink)).datosLink).accesos $\leftarrow$  *((obtener(l,s.accesosXLink)).datosLink).accesos -  ult(*((obtener(l,s.accesosXLink)).datosLink).accesos) FI} FI \\\\
  ...
  \textbf{IESRECIENTE?} (\textbf{in} s: lli, \textbf{in} l: link , \textbf{in} f: fecha) $\longrightarrow$ res: bool\\\\
	...
	$\hspace*{6mm}$res $\longleftarrow$ sumarAccesosRecientes(s, l, diasRecientesParaCategoria(s, c) $\cap$ diasRecientes(s, l)) //O(ALGO) \\\\
	...
	$\hspace*{6mm}$res $\longleftarrow$ max(fechaUltimoAcceso(s,l)+1,diasRecientes) - diasRecientes //O(ALGO)\\\\

	\textbf{IDIASRECIENTES} (\textbf{in} s: lli, \textbf{in} l: link) $\longrightarrow$ res: fecha\\\\
	   $\hspace*{6mm}$res $\longleftarrow$ diasRecientesDesde(s,l,menorReciente(s,l)) \\\\
	
	\textbf{IDIASRECIENTESDESDE} (\textbf{in} s: lli, \textbf{in} l: link) $\longrightarrow$ res: fecha\\\\
	$\hspace*{6mm}$ conj $\longleftarrow$ $\emptyset$ //O(1)\\
	$\hspace*{6mm}$ fecha $\longleftarrow$ f //O(1)\\
	$\hspace*{6mm}$ while(esReciente?(s,l,fecha))\\
	$\hspace*{6mm}$ Ag(fecha,conj)\\
	$\hspace*{6mm}$ fecha $\longleftarrow$ fecha + 1 //O(1)\\
	$\hspace*{6mm}$ endwhile\\
	$\hspace*{6mm}$ res $\longleftarrow$ conj\\\\
