...
$\hspace*{10mm}$\textit{descendientes}:dicc(\textit{padre}:string,tupla(\textit{abuelo}:string,\textit{losDeAbajo}:conj(string),\textit{id}:nat)),\\)\\

	\subsubsection{\Large Invariante de Representaci\'{o}n}
	
	  \begin{enumerate}

		\item Si la clave '\textit{padre}' de '\textit{descendientes}' es igual a '\textit{raiz}', '\textit{losDeAbajo}' sumado a '\textit{padre}' es igual al conjunto de todas las claves del dicc '\textit{descendientes}'.
		
		\item '\textit{cantidad}' sera igual a la cantidad de elementos del conjunto de todas las claves del dicc '\textit{descendientes}'.
...
\subsubsection{\Large Funci\'{o}n de Abstraccion}
	    ...
	    \textbf{Abs}: acat e $\rightarrow$ arbolDeCategorias
