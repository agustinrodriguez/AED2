...
\textbf{------------------------------------------------------------------------------\\}
\textbf{Algoritmo: 17}\\
	\textbf{IDIASRECIENTES} (\textbf{in} s: lli, \textbf{in} l: link) $\longrightarrow$ res: conj(fecha)\\
...
\textbf{------------------------------------------------------------------------------\\} 
 \textbf{Algoritmo: 18}\\
	\textbf{IDIASRECIENTESDESDE} (\textbf{in} s: lli, \textbf{in} l: link, \textbf{in} f: fecha) $\longrightarrow$ res: conj(fecha)\\
	$\hspace*{6mm}$ \textbf{while}(esReciente?(s,l,f))\\
	$\hspace*{6mm}$ Agregar(f,res) O(1)\\
	$\hspace*{6mm}$ fecha++ //O(1)\\
	$\hspace*{6mm}$ \textbf{end while}\\
...
%	$\hspace*{6mm}$ todoElConjunto $\leftarrow$ ls //O(ALGO) \\	$\hspace*{6mm}$\textbf{while}($¬\emptyset$?(todoElConjunto) $\wedge$ $¬\emptyset$(ls))\\
...
\textbf{Algoritmo: 19}\\	
	...
	$\hspace*{6mm}$ itLista(puntero(datosLink)) links $\leftarrow$ crearIt(arrayCatLinks[id(s.arbolCategorias,c)] //O(1)) \\
...
\textbf{Algoritmo: 21}\\	
	...
	$\hspace*{6mm}$puntero(datosLink) max $\leftarrow$ (siguiente(ls)) //O(1) \\
	$\hspace*{6mm}$ \textbf{while}(!haySiguiente(ls))\\
	$\hspace*{6mm}$ avanzar(ls) //O(1)\\
	$\hspace*{6mm}$ \textbf{if} (ultimo((*max).accesosRecientes)).dia < (ultimo((*siguiente(ls)).accesosRecientes)).dia \\
	$\hspace*{6mm}$ \textbf{then} max $\leftarrow$ (siguiente(ls))  //O(1) \\
	$\hspace*{6mm}$ \textbf{fi} \\
	$\hspace*{6mm}$ \textbf{end while}\\ 
	$\hspace*{6mm}$ res $\leftarrow$ (*max).link //O(|(*max).link|) \\
