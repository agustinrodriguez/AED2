...
$\hspace*{10mm}$\textit{arrayCantLinks}:arreglo-dimen(linksFamilia) )\\\\
...
\end{flushright}
  ...
  $\hspace*{6mm}$res $\leftarrow$ prim(*((obtener(l,s.accesosXLink))).accesos).dia //O(|l|) \\\\
  ...
  $\hspace*{6mm}$accesos $\leftarrow$ *((obtener(l,s.accesosXLink))).accesos O(|l|)\\
  $\hspace*{6mm}$while($¬$esVacia?(accesos) $\wedge$ (prim(accesos)).dia $\neq$ f)\\
  $\hspace*{6mm}${\IF\ (prim(accesos)).dia == f THEN res = (prim(accesos)).cantAccesos ELSE accesos = fin(accesos) FI }O(ALGO)\\
  ...
  $\hspace*{6mm}$res.arrayCantLinks[c] $\leftarrow$ 0 //O(1) \\
  $\hspace*{6mm}$c ++ //O(1) \\
  $\hspace*{6mm}$end while

  \textbf{INUEVOLINK} (\textbf{in/out} s: lli, \textbf{in} l: link , \textbf{in} c: categoria) O(ALGO) \\
  $\hspace*{6mm}$s.accesosXLink $\leftarrow$ definir(l,(c,$\emptyset$),s.accesosXlink) O(ALGO)\\
  $\hspace*{6mm}$obtener(c,s.linksXCat) $\leftarrow$ Ag(l,obtener(c,s.linksXCat)) O(ALGO)\\\\

  \textbf{IACCESO} (\textbf{in/out} s: lli, \textbf{in} l: link , \textbf{in} f: fecha) \\
  $\hspace*{6mm}$ \IF\ s.actual == f THEN s.actual $\leftarrow$ s.actual ELSE s.actual $\leftarrow$ f FI O(ALGO) \\
  $\hspace*{6mm}$ \IF\ f = (prim*((obtener(l,s.accesosXLink)).datosLink).accesos).dia \\
THEN (prim*((obtener(l,s.accesosXLink)).datosLink).accesos).dia $\leftarrow$ (prim*((obtener(l,s.accesosXLink)).datosLink).accesos).dia + 1 
ELSE {\IF\ long(*((obtener(l,s.accesosXLink)).datosLink).accesos) < 3 
THEN *((obtener(l,s.accesosXLink)).datosLink).accesos $\leftarrow$ (f,1) $\textbullet$ *((obtener(l,s.accesosXLink)).datosLink).accesos 
ELSE *((obtener(l,s.accesosXLink)).datosLink).accesos $\leftarrow$  *((obtener(l,s.accesosXLink)).datosLink).accesos -  ult(*((obtener(l,s.accesosXLink)).datosLink).accesos) FI} FI \\\\
...
$\hspace*{6mm}$		tuplaA $\leftarrow$ (c,1,1,$\emptyset$,punt)\\
...
$\hspace*{6mm}$		tuplaA $\leftarrow$ (h,ac.cantidad +1,(*puntPadre).altura +1,$\emptyset$,puntPadre) //O(|h|) \\
