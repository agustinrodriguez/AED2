...
\tadOperacion{accesos}{linkLinkIT/it, link/l}{secu(tupla($fecha, nat$))}{$l \in LinkLinkIT(it)$}
...
$\\$
\tadAxioma{accesos(agregarLink(it, l, c),l')}{$\emptyset$}
\tadAxioma{accesos(accederLink(it, l, f),l')}{\IF\ l = l' THEN agregarAcceso(accesos(it, l'), f) ELSE accesos(it,l') FI}
$\\$
$\\$
$\\$
...
$\\$
\tadAxioma{ultimoAcceso(it)}{accesoFinal(links(it))}
\tadAxioma{accesoFinal(ls)}{\IF\ $\prod_1$(prim(accesos(dameUno(ls)))) $\leq$ accesoFinal(sinUno(ls)) THEN $\prod_1$(prim(accesos(dameUno(ls)))) ELSE accesoFinal(sinUno(ls)) FI}
$\\$
