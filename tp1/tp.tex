...
\tadOperacion{accesosXDia}{linkLinkIT/it, link/l, fecha/f}{secu(nat)}{$l \in LinkLinkIT(it)$ $ \&\& $ $fechaAct(it) - f \leq 2$}
...
\tadOperacion{accesosUlt3Dias}{linkLinkIT/it, link/l}{nat}{}
...
\tadAxioma{losFamiliares(c,ac)}{\IF\ c = raiz(ac) THEN dameTodos(ac) ELSE losParientes(c,hijos(ac)) FI}
...
\tadAxioma{todosLosDeAbajo(ac)}{\IF\ $\emptyset$(ac) THEN $\emptyset$ ELSE dameTodos(dameUno(ac)) $\cup$ todosLosDeAbajo(sinUno(ac)) FI}
\tadAxioma{estaEnAlguno(ac,c)}{\IF\ c $\in$ dameTodos(ac) THEN true ELSE false FI}
$\\$
