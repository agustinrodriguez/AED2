...
\tadIgualdadObservacional{s}{s'}{secu($\alpha$)}{vac�a?($s$) $\igobs$ vac�a?($s'$) $\land$ \\($\neg$ vac�a?($s$) $\implies$ (prim($s$) $\igobs$ prim($s'$) $\land$ fin($s$) $\igobs$ fin($s'$))}
...
\tadAlinearFunciones{vac�a?~}{secu($\alpha$),secu($\alpha$)}

\tadObservadores
\tadOperacion{vac�a?}{secu($\alpha$)}{bool}{}
...
\tadAxioma{est�?($e$, $s$)}{$\neg$ vac�a?($s$) $\land$\ ($e =$ prim($s$) $\lor$ est�?($e$, fin($s$))}
...
\tadIgualdadObservacional{p}{p'}{pila($\alpha$)}{vac�a?($p$) $\igobs$ vac�a?($p'$)) $\land$ ($\neg$ vac�a?($p$) $\implies$\\ (tope($p$) $\igobs$ tope($p'$) $\land$ desapilar($p$) $\igobs$ desapilar($p'$))}
...
\tadIgualdadObservacional{c}{c'}{cola($\alpha$)}{vac�a?($c$) $\igobs$ vac�a?($c'$) $\land$ \\ 
                                                 ($\neg$vac�a?($c$) $\implies$ (pr�ximo($c$) $\igobs$ pr�ximo($c'$) $\land$ \\
                                        \phantom{($\neg$vac�a?($c$) $\implies$ (}desencolar($c$) $\igobs$ desencolar($c'$))}
...
\tadIgualdadObservacional{c}{c'}{colaPrior($\alpha$)}{vac�a?($c$) $\igobs$ vac�a?($c'$) $\land$ \\ 
                                                 ($\neg$vac�a?($c$) $\implies$ (pr�ximo($c$) $\igobs$ pr�ximo($c'$) $\land$ \\
                                        \phantom{($\neg$vac�a?($c$) $\implies$ (}desencolar($c$) $\igobs$ desencolar($c'$))}
...
\tadAxioma{pr�ximo(encolar($e$, $c$))}{\IF vac�a?($c$) $\lor$\ proximo($c$) $< e$ THEN $e$ ELSE pr�ximo($c$) FI}

\tadAxioma{desencolar(encolar($e$, $c$))}{\IF vac�a?($c$) $\lor$\ proximo($c$) $< e$ THEN $c$ ELSE encolar($e$, desencolar($c$)) FI}
